...
-:  ei
	...
	ld    hl, GlobalTriggers   ;play ending sequence MRCAT
	ld a, $07
    ld    (CurrentLevel), a
    ld a, $00
    ld    (CurrentAct), a
...
TitleScreen_ChangePressStartText:       ; $1060
   ...
   ;and   $20                        ;alternate between "Press Start Button" and blank     MRCAT -- FIX
...
;Initial positions of the status icons (lives/timer/rings) 		MRCAT
Engine_Data_StatusIconDefaults:     ;$221F
;        hpos
;   vpos  |   char
;  |----|----|----|
.db $BF, $28, $16       ;life icon position
.db $10, $10, $28
.db $10, $18, $2A
.db $BF, $20, $24
.db $10, $20, $10       ;ring icon position
.db $10, $28, $10
.db $BF, $10, $3E
.db $BF, $18, $40
.db $10, $D8, $10       ;timer position
.db $10, $E0, $26
.db $10, $E8, $10
.db $10, $F0, $10
...
Player_SetState_Jumping_Ramp:
    xor   a
    ld    ($D3AA), a        ;reset the jump hold counter
    
    ld    a, SFX_Jump         ;set spring sound effect
    bit   1, (ix+$03)     ;check to see if the player is rolling
    jp    z, Not_LongJ_SFX     ;not rolling 

    ld    a, SFX_Jump        ;set jump sound effect

Not_LongJ_SFX:

    ld    (Sound_MusicTrigger1), a	;play sound effect
...
Player_CalcGravity:         ;$3B6A
    ...
    ld    de, $0018                        ;deceleration from vertical spring			MRCAT
    cp    PlayerState_VerticalSpring
    jr    z, +
    ...
    ld    de, $0050                        ;deceleration from normal jump		MRCAT
...
SlopeJump:
    ld    hl, $F800       ;jump velocity
    ld    (Player.VelY), hl    
;jp    Player_SetState_VerticalSpring 
    jp    Player_SetState_Jumping_Ramp
	ret
Portal_Z2:				;MRCAT
    call  FadeMusic
    ret
Checkpoint_monitor:
	ld a, ($D510)	;MRCAT Save Sonic's position
	ld ($DF00), a
	ld a, ($D511)
	ld ($DF01), a
	ld a, ($D512)
	...
	ld a, ($D284)	;MRCAT Save camera position
	ld ($DF06), a
	ld a, ($D285)
	ld ($DF07), a
	ld a, ($D286)
	...
	ld a, $01	;Set the "checkpoint enabled" flag
	ld ($DF0A), a
	ret

CheckpointStuff:
	ld a, ($DF00)	;MRCAT Save Sonic's position
	ld ($D510), a
	ld a, ($DF01)
	ld ($D511), a
	ld a, ($DF02)
	...
	ld a, ($DF06)	;MRCAT Save camera position
	ld ($D284), a
	ld a, ($DF07)
	ld ($D285), a
	ld a, ($DF08)
...
Collision_Monitor_Life:     ;482A
    res   1, (hl)
    ld    a, (RingCounter)
    add   a, $20      ;add 20 rings to the counter (bcd)
    daa
    ld    (RingCounter), a
    ld    a, SFX_10Rings
    ld    (Sound_MusicTrigger1), a
...
.dw LABEL_6D0D  ;$0E
.dw Portal_Z2	;0F - Zone 2 Portal
.dw LABEL_6E30  
.dw Portal_Z3  ;11 - Zone 3 Portal
.dw Portal_Z4  ;12 - Zone 4 Portal
...
;*  player object and a level block.                *
;****************************************************
Player_CheckTopLevelCollision:          ;$72B8 BOOKMARK
    bit   1, (ix+$22)
    ret   nz
    res   0, (ix+$22)
